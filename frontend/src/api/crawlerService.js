...
const crawlerService = {
  ...
  crawlNews: async (keywords, maxNewsPerKeyword = 50) => {
    try {
      const response = await apiClient.post('/crawler/crawl', {
        keywords,
  ...
  getFiles: async () => {
    try {
      const response = await apiClient.get('/crawler/files');
  ...
  getFilePreview: async (fileName, maxRows = 5) => {
    try {
      const response = await apiClient.get(`/crawler/files/${fileName}/preview`, {
  ...
  getFileStatistics: async (fileName) => {
    try {
      const response = await apiClient.get(`/crawler/files/${fileName}/statistics`);
      return response.data;
    } catch (error) {
